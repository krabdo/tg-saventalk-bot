...
func (b *Bot) notify(ctx context.Context) {
	...
	b.mu.Unlock()
	if send {
		_ = b.API.telegram(ctx, "sendMessage", Obj{"chat_id": b.Owner, "text": "归档或 AI 出现异常。请 /status all 或在联系人话题 /status 核对；失败任务已保存，不确定的发送不会自动重试。"}, nil)
	}
}
...
func (b *Bot) record(q queryer, s *State, m Message, id int64, edited bool, p Policy) {
	...
	current := len(old) == 0 || old[0]["body"] == nil || version > integer(old[0]["version"]) || (version == integer(old[0]["version"]) && id > integer(old[0]["update_id"]))
	if len(old) > 0 {
		anchor = integer(old[0]["anchor"])
		deleted = integer(old[0]["deleted"]) != 0
	}
	if current {
		exec(q, `INSERT INTO messages(chat,key,body,version,update_id) VALUES(?,?,?,?,?) ON CONFLICT(chat,key) DO UPDATE SET body=excluded.body,version=excluded.version,update_id=excluded.update_id`, s.Chat, key, raw(m), version, id)
		s.Revision++
	}
	...
	if m.owner(b.Owner) && m.SenderBot == nil && !edited {
		...
		s.FirstPending = 0
		s.Revision++
	} else if !m.owner(b.Owner) && m.SenderBot == nil && (m.From == nil || !m.From.Bot) && !edited && !deleted {
...
func (b *Bot) ingest(q queryer, u Update) {
	...
	if m != nil {
		...
		for _, id := range u.Deleted.IDs {
			...
			old := rows(q, "SELECT body,anchor FROM messages WHERE chat=? AND key=?", s.Chat, key)
			text := fmt.Sprintf("🗑 原消息 #%d 已删除；未收到原消息", id)
			body := Obj{}
			if len(old) > 0 && old[0]["body"] != nil {
			...
			body["text"] = text
			exec(q, "INSERT INTO messages(chat,key,deleted) VALUES(?,?,1) ON CONFLICT(chat,key) DO UPDATE SET deleted=1", s.Chat, key)
